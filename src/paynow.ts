		...
		if (!this.success) {
			...
			if (typeof data.authorizationcode !== "undefined") {
				...
				this.innbucks_info.push({
					...
					deep_link_url:
						INNBUCKS_DEEPLINK_PREFIX + data.authorizationcode,
...
export class Paynow {
	...
	init(payment: Payment) {
		...
		return http({
			...
			.catch(function (err) {
				console.log(
					"An error occured while initiating transaction",
					err
				);
			});
	}
	...
	isValidEmail(emailAddress: string) {
		...
		// Updated regex to allow for a wider range of TLDs
	...
	parse(response: Response) {
		...
		if (response) {
			let parsedResponseURL = this.parseQuery(
	...
	build(payment: Payment) {
		let data: { [key: string]: string } = {
			...
			authemail:
				typeof payment.authEmail === "undefined"
					? ""
					: payment.authEmail,
			status: "Message",
		};
